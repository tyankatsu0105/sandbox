...
import * as Presentation from './update-one.presentation';
...
// schema
// ===============================
const schema = yup.object().shape<UsersEntity.FormData['updateOne']>({
...
export const Component = (props): React.ReactElement => {
  ...
  const onSubmit = hookFormMethods.handleSubmit((data) => {
    dispatch(EntitiesUsers.actions.updateUser({ id: data.id, changes: data }));
  });
