...
export const fetchMovies = createAsyncThunk<
  Types.Payload['operation']['fetchMovies'],
  {
    ids: Entity.Book['id'][];
  },
  AsyncThunkConfig<{ message: string }>
>(`${name}/fetchMovies`, async (args, thunkAPI) => {
  try {
    const { data } = await thunkAPI.extra.api.query<
      GraphQLTypes.UserFavoriteMoviesQuery,
      GraphQLTypes.UserFavoriteMoviesQueryVariables
    >({
      query: GraphQLTypes.UserFavoriteMoviesDocument,
      variables: { ids: args.ids, page: { first: args.ids.length } },
    });

    return new Repository.UserFavoriteMoviesRepository(data)
      .toEntityUserFavoriteMovies;
  } catch (error) {
